...
- Sparse data가 가진 차원의 저주 문제를 풀기 위한 시도 중 하나로는, 특징($P$)의 수를 줄이려는 시도가 있다.
- 이를 위해서는 특징들 중에서도 ML 훈련에 사용하기 좋은 경향성이 뚜렷한 특징들만 선택하는 방법이라던가,
- 알고리즘을 활용하여 여러 특징들을 병합하여 하나로 만드는 방법이라던가,
- 혹은 방금 말한 두 가지를 아예 한꺼번에 적용하는 방법이 있다.
